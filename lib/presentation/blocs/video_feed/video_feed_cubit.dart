...
class VideoFeedCubit extends Cubit<VideoFeedState> {
  ...
  Future<void> toggleVideoProperty(String docId, VideoPropertyEnums videoProperty, bool newValue) async {
    try {
      // Update Firestore via repository.
      await videoRepository.updateVideoProperty(docId, videoProperty, newValue);
            ...
            if (video.id == docId) {
              if (videoProperty == VideoPropertyEnums.like) {
                return VideoItem(
                  ...
                  likeCount: newValue ? video.likeCount + 1 : (video.likeCount > 0 ? video.likeCount - 1 : 0),
